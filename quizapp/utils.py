...
def load_questions(section_id, amount=20):
    ...
    section_categories = Category.objects.filter(section=section_id).values_list('id', flat=True)
    question_ids = list(Question.objects.filter(category__in = section_categories)[:amount])
